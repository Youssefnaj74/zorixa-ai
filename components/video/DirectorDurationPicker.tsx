...
export function DirectorDurationPicker({
  options,
  value,
  onChange,
  className
  ...
  const displaySec = options.includes(value) ? value : (options[0] ?? value);
  ...
  return (
    <div ref={rootRef} className={cn("relative inline-flex items-center", className)}>
      <button
        ...
        aria-label={canPick ? `Duration ${displaySec} seconds` : `Duration fixed at ${displaySec} seconds`}
        className={cn(
          triggerClass,
      ...
      >
        <span className="tabular-nums">{displaySec}s</span>
        {canPick ? (
          <ChevronUp
            className={cn(
      ...
      <AnimatePresence>
        {open && canPick ? (
          <motion.div
            ...
            className={cn(dropupPanelClass, "left-0 max-h-52 min-w-[88px] overflow-y-auto py-1")}
            role="listbox"
